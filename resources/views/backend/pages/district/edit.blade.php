...
<div class="form-group">
    ...
    <input type="text" name="name" class="form-content" required="required" value="{{$district->name}}">
</div>

<div class="form-group">
<label>District Description</label>
<textarea class="form-control" name="description" rows="7">{{$district->description}}</textarea>
</div>
</div>
...
<option value="1" @if($district->is_featured==1)selected @endif>Yes Featured</option>

<option value="0" @if($district->is_featured==0)selected @endif>Not Featured</option>
</select>
</div>
...
<option value="1" @if($district->status==1)
selected
@endif>Active</option>

<option value="0" @if($district->status==0)
selected
...
<div class="form-group">
<label>District logo/Image</label><br>
@if(!is_null($district->image))
<img src="{{asset('backend/img/district')}}/{{$district->image}}" width="80">
@else
No Thumbnail
@endif
<br><br>
